 ...
 * supports both non-stream and stream (SSE) responses.
 */
async function callChat(payload: any, opts?: { stream?: boolean; signal?: AbortSignal }) {
  const r = await fetch('/api/chat', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ ...payload, stream: !!opts?.stream }),
    signal: opts?.signal,
  });
...
/**
 * SSE reader: expects the server to emit lines like:
 * data: {"type":"delta","text":"..."}
 * data: {"type":"done"}
 */
async function* streamTextFromSSE(response: Response): AsyncIterable<string> {
...
function App() {
  ...
  // Dynamic placeholder generation on load (now via /api/chat)
  useEffect(() => {
...
\`{ "name": "Persona Name", "html": "..." }\`
      ...
      // stream variations as SSE deltas
      ...
      for await (const variation of parseJsonStream(streamTextFromSSE(resp))) {
...
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions (e.g. ["Tactile Risograph Press", "Kinetic Silhouette Balance", "Primary Pigment Gridwork"]).
      ...
      const styleResp = await callChat({
        model: VENICE_MODEL,
      ...
      const styleText = styleData?.choices?.[0]?.message?.content || '[]';
...
Return ONLY RAW HTML. No markdown fences.
          ...
          let accumulatedHtml = '';
          for await (const text of streamTextFromSSE(resp)) {
  ...
  }, [inputValue, isLoading, sessions.length, placeholderIndex, placeholders]);
